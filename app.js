(() => {
  ...
  function loadState() {
    try {
      ...
      parsed.players = parsed.players.map((player) => ({ ...player, pending: Array.isArray(player.pending) ? player.pending : [] }));
  ...
  function normalizeBackup(payload) {
    ...
    copy.players = copy.players.map((player) => {
      ...
      return {
        ...
        goalkeeper,
        attrs,
        pending: Array.isArray(player.pending)
          ? player.pending.filter((item) => item && typeof item.matchId === "string" && (item.delta === 18 || item.delta === -18))
          : [],
  ...
  function adjustmentContext(player) {
    ...
    const target = Math.min(18, capacity);
    const used = attributes.reduce((sum, item) => sum + (adjustmentDraft[item.key] || 0), 0);
    return { pending, sign, attributes, target, used, remaining: target - used };
  }
    ...
    return `
      <section class="pending-adjustment ${win ? "" : "loss-adjustment"}">
        <div class="adjustment-head">
          <div>
            <h3>* ${win ? "Vitória: distribua +18 pontos" : "Derrota: remova 18 pontos"}</h3>
            <p>${match ? `Partida #${matchNumber(match)} · ${match.scoreA} × ${match.scoreB}` : "Ajuste pendente"}${context.target < 18 ? ` · limite aplicável: ${context.target}` : ""}</p>
          </div>
  ...
  function balanceTeams() {
    const sorted = [...state.players].sort((a, b) => overall(b) - overall(a) || a.name.localeCompare(b.name, "pt-BR"));
    const maxA = Math.ceil(sorted.length / 2);
    const maxB = Math.floor(sorted.length / 2);
    const teamA = [];
    const teamB = [];
    let totalA = 0;
    let totalB = 0;
    ...
    sorted.forEach((player) => {
      const value = overall(player);
      if (teamA.length >= maxA) { teamB.push(player); totalB += value; return; }
      if (teamB.length >= maxB) { teamA.push(player); totalA += value; return; }
      if (totalA <= totalB) { teamA.push(player); totalA += value; }
      else { teamB.push(player); totalB += value; }
    });

    let improved = true;
    while (improved) {
      improved = false;
      let best = Math.abs(totalA - totalB);
      for (let a = 0; a < teamA.length; a += 1) {
        for (let b = 0; b < teamB.length; b += 1) {
          const nextA = totalA - overall(teamA[a]) + overall(teamB[b]);
          const nextB = totalB - overall(teamB[b]) + overall(teamA[a]);
          const difference = Math.abs(nextA - nextB);
          if (difference + 0.01 < best) {
            const temp = teamA[a]; teamA[a] = teamB[b]; teamB[b] = temp;
            totalA = nextA; totalB = nextB; best = difference; improved = true;
          }
        }
      }
    }

    return { teamA: teamA.map((player) => player.id), teamB: teamB.map((player) => player.id), excluded: [], scoreA: "", scoreB: "" };
  }
  ...
  function generateDraft() {
    ...
    setScreen("matches");
    renderMatches();
    showToast("Lados equilibrados pelo overall do elenco.");
  }
  ...
  function confirmMatch() {
    ...
    [...match.teamA, ...match.teamB].forEach((playerId) => {
      const player = state.players.find((item) => item.id === playerId);
      if (player) player.pending.push({ matchId: match.id, delta: winners.includes(playerId) ? 18 : -18 });
    });
    state.draft = null;
    saveState();
    renderAll();
